...
def discriminator(image, options, reuse=False, name="discriminator", layers=3):
    ...
    with tf.variable_scope(name):
        ...
        for i in range(layers):
            df_dim *= 2
            conv_name = "d_h%d_conv" % (i+1)
            norm_name = "d_bn%d" % (i+1)
            h = lrelu(instance_norm(conv2d(h, df_dim, name=conv_name), norm_name))
        ...
        conv_name = "d_h%d_pred" % layers
        h4 = conv2d(h, 1, s=1, name=conv_name)
        # h4 is (32 x 32 x 1)
        return h4
...
def seg_generator_resnet(image, options, output_dim = 1, reuse=False, name="seg_generator"):
    with tf.variable_scope(name):
        # image is 256 x 256 x input_c_dim
        if reuse:
            tf.get_variable_scope().reuse_variables()
        ...
        d1 = deconv2d(r7, options.gf_dim*2, 3, 2, name='g_d1_dc')
        d1 = tf.nn.relu(instance_norm(d1, 'g_d1_bn'))
        d2 = deconv2d(d1, options.gf_dim, 3, 2, name='g_d2_dc')
        d2 = tf.nn.relu(instance_norm(d2, 'g_d2_bn'))
        d2 = tf.pad(d2, [[0, 0], [3, 3], [3, 3], [0, 0]], "REFLECT")
        seg = tf.nn.tanh(conv2d(d2, output_dim, 7, 1, padding='VALID', name='g_seg_c'))
